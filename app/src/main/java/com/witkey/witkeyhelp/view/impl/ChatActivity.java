...
import com.witkey.witkeyhelp.adapter.ChattingListAdapter;
import com.witkey.witkeyhelp.util.ImageItem;
...
import com.witkey.witkeyhelp.util.wight.EmoticonsEditText;
import com.witkey.witkeyhelp.util.wight.EmoticonsKeyboardUtils;
import com.witkey.witkeyhelp.util.wight.FuncLayout;
import com.witkey.witkeyhelp.util.wight.ImageEvent;
import com.witkey.witkeyhelp.util.wight.XhsEmoticonsKeyBoard;

import com.witkey.witkeyhelp.view.impl.base.BaseMessageToActivity;
import com.witkey.witkeyhelp.widget.ChatView;
import com.witkey.witkeyhelp.widget.Constants;
import com.witkey.witkeyhelp.widget.IdHelper;
import com.witkey.witkeyhelp.widget.SimpleAppsGridView;
import com.witkey.witkeyhelp.widget.TipItem;
import com.witkey.witkeyhelp.widget.TipView;
import com.witkey.witkeyhelp.widget.listview.DropDownListView;
import com.witkey.witkeyhelp.widget.pickerimage.PickImageActivity;
...
import com.witkey.witkeyhelp.widget.pickerimage.utils.SendImageHelper;
import com.witkey.witkeyhelp.widget.pickerimage.utils.StorageType;
import com.witkey.witkeyhelp.widget.pickerimage.utils.StorageUtil;
import com.witkey.witkeyhelp.widget.pickerimage.utils.StringUtil;
import com.witkey.witkeyhelp.widget.takevideo.CameraActivity;
...
public class ChatActivity extends BaseMessageToActivity implements FuncLayout.OnFuncKeyBoardListener, View.OnClickListener {
    ...
    private void initData() {
        ...
        mTitle = intent.getStringExtra(MyAPP.CONV_TITLE);
...
//        startActivityForResult(intent, MyAPP.REQUEST_CODE_CHAT_DETAIL);
    }
...
//                    }
    ...
    protected void onResume() {
        ...
        if (!mIsSingle) {
            ...
            if (groupId != 0) {
                MyAPP.isAtMe.put(groupId, false);
                MyAPP.isAtall.put(groupId, false);
...
//                    Toast.makeText(this, "请在应用管理中打开“读写存储”访问权限！", Toast.LENGTH_LONG).show();
                    ActivityCompat.requestPermissions(this, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE}, MyAPP.IMAGE_MESSAGE);
                } else {
                    PickImageActivity.start(ChatActivity.this, requestCode, from, tempFile(), true, 9,
                            true, false, 0, 0);
                }
                break;
            case MyAPP.TAKE_PHOTO_MESSAGE:
...
//                    Toast.makeText(this, "请在应用管理中打开“相机,读写存储,录音”访问权限！", Toast.LENGTH_LONG).show();
                    ActivityCompat.requestPermissions(this, new String[]{Manifest.permission.CAMERA, Manifest.permission.WRITE_EXTERNAL_STORAGE, Manifest.permission.RECORD_AUDIO},MyAPP.TAKE_PHOTO_MESSAGE);
                } else {
                    intent = new Intent(ChatActivity.this, CameraActivity.class);
                    startActivityForResult(intent, RequestCode.TAKE_PHOTO);
                }
                break;
            case MyAPP.TAKE_LOCATION:
...
//                    Toast.makeText(this, "请在应用管理中打开“位置”访问权限！", Toast.LENGTH_LONG).show();
                    ActivityCompat.requestPermissions(this, new String[]{Manifest.permission.ACCESS_FINE_LOCATION}, MyAPP.TAKE_LOCATION);

                } else {
//                    intent = new Intent(mContext, MapPickerActivity.class);
//                    intent.putExtra(MyAPP.TARGET_ID, mTargetId);
//                    intent.putExtra(MyAPP.TARGET_APP_KEY, mTargetAppKey);
//                    intent.putExtra(MyAPP.GROUP_ID, mGroupId);
//                    intent.putExtra("sendLocation", true);
//                    startActivityForResult(intent, MyAPP.REQUEST_CODE_SEND_LOCATION);
                }
                break;
            case MyAPP.FILE_MESSAGE:
                if (ContextCompat.checkSelfPermission(this,
                        Manifest.permission.WRITE_EXTERNAL_STORAGE)
                        != PackageManager.PERMISSION_GRANTED) {
//                    Toast.makeText(this, "请在应用管理中打开“读写存储”访问权限！", Toast.LENGTH_LONG).show();
                    ActivityCompat.requestPermissions(this, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE}, MyAPP.FILE_MESSAGE);
...
//                startActivity(intent);
                break;
            case MyAPP.TACK_VIDEO:
            case MyAPP.TACK_VOICE:
                ToastUtils.showTestShort(mContext, "该功能正在添加中");
                break;
            default:
                break;
        }
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == MyAPP.IMAGE_MESSAGE) {
...
//                        true, false, 0, 0);
        ...
        } else if (requestCode == MyAPP.TAKE_PHOTO_MESSAGE) {
            Log.e("tag","123456");
            if (grantResults[0] != PackageManager.PERMISSION_GRANTED) {
        ...
        } else if (requestCode == MyAPP.TAKE_LOCATION) {
            if (grantResults[0] != PackageManager.PERMISSION_GRANTED) {
                Toast.makeText(this, "请在应用管理中打开“位置”访问权限！", Toast.LENGTH_LONG).show();
            } else {
...
//                startActivityForResult(intent, JGApplication.REQUEST_CODE_SEND_LOCATION);
        ...
        } else if (requestCode == MyAPP.FILE_MESSAGE) {
            if (grantResults[0] != PackageManager.PERMISSION_GRANTED) {
                Toast.makeText(this, "请在应用管理中打开“读写存储”访问权限！", Toast.LENGTH_LONG).show();
            } else {
...
//                startActivityForResult(intent, JGApplication.REQUEST_CODE_SEND_FILE);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (resultCode) {
            case MyAPP.RESULT_CODE_AT_MEMBER:
                if (!mIsSingle) {
                    GroupInfo groupInfo = (GroupInfo) mConv.getTargetInfo();
                    String username = data.getStringExtra(MyAPP.TARGET_ID);
                    String appKey = data.getStringExtra(MyAPP.TARGET_APP_KEY);
                    ...
                    mLongClick = true;
                    ekBar.getEtChat().appendMention(data.getStringExtra(MyAPP.NAME));
                    ekBar.getEtChat().setSelection(ekBar.getEtChat().getText().length());
                }
                break;
            case MyAPP.RESULT_CODE_AT_ALL:
                mAtAll = data.getBooleanExtra(MyAPP.ATALL, false);
                mLongClick = true;
                if (mAtAll) {
            ...
            case RequestCode.TAKE_VIDEO:
                ...
                break;
//            case MyAPP.RESULT_CODE_SEND_LOCATION:
...
//                mChatView.setToBottom();
//                break;
            case MyAPP.RESULT_CODE_SEND_FILE:
                int[] intArrayExtra = data.getIntArrayExtra(MsgIDs);
                for (int msgId : intArrayExtra) {
                    handleSendMsg(msgId);
                }
                break;
            case MyAPP.RESULT_CODE_CHAT_DETAIL:
                String title = data.getStringExtra(MyAPP.CONV_TITLE);
                if (!mIsSingle) {
                    GroupInfo groupInfo = (GroupInfo) mConv.getTargetInfo();
                    UserInfo userInfo = groupInfo.getGroupMemberInfo(mMyInfo.getUserName(), mMyInfo.getAppKey());
                    //如果自己在群聊中，同时显示群人数
        ...
        ImageContent.createImageContentAsync(file, new ImageContent.CreateImageContentCallback() {
            ...
            public void gotResult(int responseCode, String responseMessage, ImageContent imageContent) {
                if (responseCode == 0) {
                    ...
                    ToastUtils.showTestShort(mContext, responseMessage);
                }
            }
        });
    }
