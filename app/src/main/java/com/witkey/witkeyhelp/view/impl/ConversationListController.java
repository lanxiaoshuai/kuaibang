...
import com.witkey.witkeyhelp.adapter.ConversationListAdapter;
...
//                break;
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        if (position > 0) {
            ...
            intent.putExtra(MyAPP.CONV_TITLE, conv.getTitle());
            //群聊
...
//                intent.putExtra(MyAPP.GROUP_ID, groupId);
//                intent.putExtra(MyAPP.DRAFT, getAdapter().getDraft(conv.getId()));
...
//            } else {
            ...
            intent.putExtra(MyAPP.TARGET_ID, targetId);
            intent.putExtra(MyAPP.TARGET_APP_KEY, conv.getTargetAppKey());
            intent.putExtra(MyAPP.DRAFT, getAdapter().getDraft(conv.getId()));
            //  }
            intent.setClass(mContext.getActivity(), ChatActivity.class);
