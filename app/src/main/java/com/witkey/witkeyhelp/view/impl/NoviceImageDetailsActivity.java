...
public class NoviceImageDetailsActivity extends BaseActivity {
    ...
    protected void onCreateActivity() {
        setContentView(R.layout.activity_long_graph);
        setIncludeTitle("新手帮助");
        imageView = findViewById(R.id.long_graph);

        // Glide.with(this).load(getIntent().getIntExtra("image", 0)).into(imageView);
      //  imageView.setImageResource(getIntent().getIntExtra("image", 0));
        Bitmap bitmap = BitmapFactory.decodeResource(getResources(), getIntent().getIntExtra("image", 0), null);
        setBitmapToImg(bitmap);
    }
