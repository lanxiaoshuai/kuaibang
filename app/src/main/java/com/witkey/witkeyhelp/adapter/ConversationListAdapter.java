...
import com.witkey.witkeyhelp.entity.Event;
...
import com.witkey.witkeyhelp.util.SortConvList;
import com.witkey.witkeyhelp.util.SortTopConvList;
import com.witkey.witkeyhelp.util.ThreadUtil;
import com.witkey.witkeyhelp.util.TimeFormat;
import com.witkey.witkeyhelp.util.UIUtils;
import com.witkey.witkeyhelp.util.ViewHolder;
import com.witkey.witkeyhelp.widget.ConversationListView;
import com.witkey.witkeyhelp.widget.RoundImageView;
import com.witkey.witkeyhelp.widget.SwipeLayoutConv;
...
public class ConversationListAdapter extends BaseAdapter {
   ...
   //     mConversationListView.setUnReadMsg(JMessageClient.getAllUnReadMsgCount());
        ...
        final RoundImageView headIcon = ViewHolder.get(convertView, R.id.msg_item_head_icon);
...
//                //会话界面时间
                ...
                if (mAtAll.get(position) && MyAPP.isAtall.get(gid) != null && MyAPP.isAtall.get(gid)) {
                ...
                } else if (mArray.get(position) && MyAPP.isAtMe.get(gid) != null && MyAPP.isAtMe.get(gid)) {
                    //有人@我 文字提示
                    ...
                    if (lastMsg.getTargetType() == ConversationType.group && !contentStr.equals("[群成员变动]")) {
                        ...
                        if (MyAPP.isAtall.get(gid) != null && MyAPP.isAtall.get(gid)) {
                            content.setText("[@所有人] " + fromName + ": " + contentStr);
                        } else if (MyAPP.isAtMe.get(gid) != null && MyAPP.isAtMe.get(gid)) {
        ...
        if (convItem.getUnReadMsgCnt() > 0) {
            ...
            if (convItem.getType().equals(ConversationType.single)) {
                if (mUserInfo != null && mUserInfo.getNoDisturb() == 1) {
                  ...
                  //  EventBus.getDefault().post(new UnreadBean("1"));
                }
            ...
            newGroupMsgNumber.setVisibility(View.GONE);
            newMsgNumber.setVisibility(View.GONE);
         //   EventBus.getDefault().post(new UnreadBean("0"));
        }
